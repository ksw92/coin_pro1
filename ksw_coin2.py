...
stock = "xoxb-3566443758993-3574327487846-Te6mTzErJS6YnjW2rPjVK2dg"
...
def get_ma15(ticker):
    """15 이동평균선 값"""
    conin_OHLCV = pyupbit.get_ohlcv(ticker, interval="day", count=15)
...
def get_ma50(ticker):
    """50 이동평균선 값"""
    conin_OHLCV = pyupbit.get_ohlcv(ticker, interval="day", count=50)
...
buy_check = 0
sell_check = 0
post_message(stock, "#stock", "[%s] - AUTO UPBIT START")
################################################
while True:
    try:
        start_time = get_start_time("KRW-XRP")           # 시작시간
        current_time = datetime.datetime.now()           # 현시간
        target_price = get_target_price("KRW-XRP", 0.3)  # 목표가(매수)
        targetB_price = get_targetB_price("KRW-XRP", 0.3)# 목표가(매도)
        ...
        sell_price = get_balance("XRP")                  # 내XRP
        ...
        if buy_check == 1:
            ...
            if targetB_price[1] >= current_price :
